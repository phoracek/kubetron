...
func (l Lister) Discover(pluginListCh chan dpm.PluginNameList) {
	pluginListCh <- dpm.PluginNameList{l.ReservedMainResourceName, l.ReservedOverlayResourceName}

	for {
		localNetworks := listLocalNetworks()
		resources := append(localNetworks, l.ReservedMainResourceName, l.ReservedOverlayResourceName)
...
func (l Lister) NewPlugin(resourceName string) dpm.PluginInterface {
	if resourceName == l.ReservedMainResourceName {
		return DevicePlugin{}
	} else {
		return AllocationDevicePlugin{}
	}
}
